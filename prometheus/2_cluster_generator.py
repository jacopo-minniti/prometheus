...
from prometheus.utm_coordinates import convert_to_utm
...
def validate_and_clean_data(df):
    ...
    numeric_columns = ['latitude', 'longitude', 'scan', 'track', 
                      'bright_ti4', 'frp'] # change FIRMS features 
...
def cluster_fires(df, spatial_threshold, temporal_threshold, min_neighbors):
    ...
    data = convert_to_utm(data, src_epsg=4326, dst_epsg=3310, col_lat='latitude', col_lon='longitude') # select EPSG code for California. Obtain EPSG code: https://epsg.io/
...
clustered_data['acq_date'] = pd.to_datetime(clustered_data['acq_date'])  
year = clustered_data['acq_date'].dt.year.iloc[0]  
month = clustered_data['acq_date'].dt.month_name().str[:3].str.lower().iloc[0] 
start_day = clustered_data['acq_date'].min().day  
end_day = clustered_data['acq_date'].max().day  

country = clustered_data['country_id'].iloc[0].lower()  
file_name = f"clustered_firms_df_{country}_{month}_{start_day}-{end_day}_{year}.csv"
clustered_data.to_csv(f"../output/{file_name}", index=False)

print(f"File saved as: ../output/{file_name}")
