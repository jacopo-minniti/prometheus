...
labels_tensor = torch.load('../data/labels_all.pt', weights_only=False).float()
features_tensor = torch.load('../data/features_all.pt', weights_only=False).float()
...
input_dim = 5  # Number of feature channels
hidden_dim = 64
output_dim = 1
kernel_size = 3
num_layers = 2
...
with torch.no_grad():
    ...
    sample_idx = 0
    prediction_map = predicted[sample_idx, 0, :, :].cpu().numpy()  # Predicted map
    ground_truth_map = labels[sample_idx, 0, :, :].cpu().numpy()  # Ground truth map
