...
def main():
    viirs = pd.read_csv("../dataset/viirs_s_npp_italy_2012_2022_clustered.csv")
    viirs = validate_and_clean_data(viirs)
    features = xr.open_dataset('../dataset/features.nc')

    labels, features = create_tensors(viirs, features)
    torch.save(torch.from_numpy(labels), '../data/labels_all.pt')
    torch.save(torch.from_numpy(features), '../data/features_all.pt')
